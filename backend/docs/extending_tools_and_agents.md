...
* **Planner-filled parameters cut the entry parse.** Templates can declare `parameters` (see `docs/sub_agents.md` § Planner-fillable parameters): the orchestrator LLM fills them when it invokes the agent, the values seed `variables`, and the entry parse_node skips (`[parse_node_skipped.v1]`) or narrows (`[parse_node_narrowed.v1]`) its LLM call — but **only on the Planner-entry pass**. Resume passes always parse in full, so everything in this section still applies to interrupt replies. Interrupts whose slot may be pre-filled must opt in with `planner_fillable: true`; confirmation gates must not, which keeps them structurally unreachable to the Planner.
