...
def _state_context(state: dict) -> dict:
    return {
...
async def dispatch_one(
    *,
    ...
    """Look up an AgentTool and dispatch one action. Returns the tool's
    result on success, or an ERROR-shaped dict for missing tool / missing
    action / dispatch exception. Never raises.

    Shared by tool_call_node and parallel_tools_node so they speak the same
    dispatch contract (lookup → dispatch → ERROR shape).

    `origin_label` is only used in error messages — pass the template's
    original tool name (which may be a legacy alias) so the error message
    matches what the author wrote.
    """
    from app.tools.agent_tool import get_agent_tool

    tool = get_agent_tool(tool_name, agent_name)
    if tool is None:
        logger.error(
    ...
    if not action:
        return {
            "status": "ERROR",
            "error_category": "system",
            "error": f"tool_call {(origin_label or tool_name)!r} missing action",
            "user_facing_message": "Internal configuration error.",
        }

    try:
        return await tool.dispatch(action, resolved_params, context)
