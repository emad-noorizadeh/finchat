...
| `OPENAI_EMBEDDINGS_TIKTOKEN_ENABLED` | `true` | `true` uses langchain's `OpenAIEmbeddings`, which pre-counts tokens via tiktoken (downloads BPE files from `openaipublic.blob.core.windows.net` once). `false` swaps in a thin OpenAI-SDK adapter that does **zero** client-side tokenization — neither tiktoken nor the transformers fallback. Use `false` on airgapped corp networks that can't reach Azure blob storage. Safe because our pipeline chunks via character-based splitter upstream, so payloads always fit the 8191-token API limit. |
...
## Offline setup (GitHub blocked on your network)

If you can't clone from GitHub — e.g., the destination server blocks
github.com — use `scaffold.sh` to create the empty folder structure
locally, then paste each file's content from the GitHub web view:

```bash
./scaffold.sh [target-dir]   # default target: ./finchat
```

This creates all 211 tracked files as zero-byte placeholders under the
target directory. Open each file in your editor, grab its content from
`https://github.com/emad-noorizadeh/finchat/blob/main/<path>`, and save.
Once every file is populated, run `./setup.sh` as normal.
